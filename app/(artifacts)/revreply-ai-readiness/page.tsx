      ...
      {unlocked && data ? (
        ...
        <section className="rr-gate" aria-labelledby="invitation-title">
          <Grid>
            ...
            <Column lg={6} md={8} sm={4}><Tile className={`rr-gate-card ${shaking ? "rr-shake" : ""}`} onAnimationEnd={() => setShaking(false)}><Stack gap={6}><Locked size={28} /><div><p className="rr-eyebrow">Your invitation</p><h2>Welcome, RevReply.</h2><p>Use the password shared with you to open the assessment.</p></div><form onSubmit={submit}><Stack gap={5}><PasswordInput id="invitation-password" labelText="Invitation password" value={password} autoComplete="off" invalid={invalid} disabled={busy} invalidText={<span role="alert">That password does not match. Please try again.</span>} onChange={event => { setPassword(event.target.value); setInvalid(false); }} required /><Button type="submit" renderIcon={ArrowRight} disabled={busy}>{busy ? "Opening…" : "Open the assessment"}</Button></Stack></form><p className="rr-caption">Need the password? Ask the person who sent you this link.</p></Stack></Tile></Column>
          </Grid>
