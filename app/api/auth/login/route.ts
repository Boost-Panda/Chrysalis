...
// Enforces the @boostpanda.ai restriction server-side; Supabase itself
...
export async function POST(request: Request) {
  ...
  if (!email || !email.endsWith("@boostpanda.ai")) {
    return NextResponse.json({ error: "only @boostpanda.ai addresses can sign in" }, { status: 403 });
  }
